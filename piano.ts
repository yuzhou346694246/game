...
var rangeInput:HTMLInputElement;
var attenuationInput:HTMLInputElement;
var delayInput:HTMLInputElement;
var span:HTMLSpanElement;
var dd:HTMLSpanElement;
var ad:HTMLSpanElement;
var toneTable={};
...
function convert(data:Array<number>){
    let ret = [];
    let t = ["","C","D","E","F","G","A","B"];
    for(let d of data){
        if(d == 0){
...
function init_piano(){
    ...
    rangeInput = <HTMLInputElement>document.getElementById("soundValue");
    span = <HTMLSpanElement>document.getElementById("dsv");
    delayInput = <HTMLInputElement>document.getElementById("delay");
    attenuationInput = <HTMLInputElement>document.getElementById("attenuation");
    dd = <HTMLSpanElement>document.getElementById("dd");
    ad = <HTMLSpanElement>document.getElementById("ad");
    btn = <HTMLButtonElement>document.getElementById("play");
    let btn1 = <HTMLButtonElement>document.getElementById("play1");
    let btnplay = <HTMLButtonElement>document.getElementById("playinput");
    let ta = <HTMLTextAreaElement>document.getElementById("inputmusic");
    ...
    delayInput.onchange = (event:Event)=>{
        let target = <HTMLInputElement>event.target;
        dd.innerHTML = target.value;
    }
    attenuationInput.onchange = (event:Event)=>{
        let target = <HTMLInputElement> event.target;
    ...
    btn1.onclick=()=>{//本按钮用来播放数字谱
        let m = new Music(testmusic);
        m.playnums(testmusic1);
    }
    btnplay.onclick=()=>{//本按钮用来播放textarea中的谱
        let notes = ta.value.split(/\s+/);
        let nn:Array<number> = notes.map((v)=>parseInt(v));
